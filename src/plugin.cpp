...
std::string checkTag(const VSMap* props, const VSAPI* vsapi, const char* key,
    ...
    if (err != 0 || value == expected) return std::string();
...
bool readMasteringLuminance(const VSMap* props, const VSAPI* vsapi, const char* key,
    ...
    int err = 0;
    const double value = vsapi->mapGetFloat(props, key, 0, &err);
...
std::string resolveFrameParams(const FilterData* d, const VSMap* props,
    ...
    if (!d->haveSrcMax &&
        !readMasteringLuminance(props, vsapi, "MasteringDisplayMaxLuminance", true,
                                &srcMax)) {
        return "src_max was not given and MasteringDisplayMaxLuminance is not usable. "
               "Pass src_max explicitly; BT.2408 names 10000 as the fallback when the "
               "mastering display peak is unknown";
    }
    if (!d->haveSrcMin &&
        !readMasteringLuminance(props, vsapi, "MasteringDisplayMinLuminance", false,
                                &srcMin)) {
        return "src_min was not given and MasteringDisplayMinLuminance is not usable. "
               "Pass src_min explicitly; BT.2408 names 0 as the fallback when the "
               "mastering display black is unknown";
    }

    out->nominal = d->nominal;
    out->dstMax = d->dstMax;
    return tonemapper::makeEetf(srcMin, srcMax, d->dstMin, d->dstMax, &out->curve);
}
...
void VS_CC bt2390Create(const VSMap* in, VSMap* out, void*, VSCore* core,
    ...
    if (!std::isfinite(d->nominal) || d->nominal <= 0.0) {
        delete d;
        fail("nominal_luminance must be positive");
        return;
    }

    // Whatever does not depend on the frame properties is checked once here,
    // so a bad dst_min fails at script evaluation rather than on frame 0. The
    // source range is only known per frame when it comes from a property.
    if (d->haveSrcMin && d->haveSrcMax) {
        tonemapper::Eetf unused{};
        const std::string bad =
            tonemapper::makeEetf(d->srcMin, d->srcMax, d->dstMin, d->dstMax, &unused);
        if (!bad.empty()) {
            delete d;
            fail(bad);
            return;
